...
func NewCometAPI(cl client.CometRPC) CometAPI {
	return cometAPIImpl{cl: cl}
}

type cometAPIImpl struct {
	cl client.CometRPC
}

// IsValidator returns true if the given address is a validator at the latest height.
// It is best-effort, so returns false on any error.
func (a cometAPIImpl) IsValidator(ctx context.Context, valAddress common.Address) bool {
...
// Validators returns the cometBFT validators at the given height or false if not
// available (probably due to snapshot sync after height).
func (a cometAPIImpl) Validators(ctx context.Context, height int64) (*cmttypes.ValidatorSet, bool, error) {
