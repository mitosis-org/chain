...
type StartCmdOptions struct {
	...
	StartCommandHandler func(svrCtx *server.Context, clientCtx client.Context, appCreator types.AppCreator, inProcessConsensus bool, opts StartCmdOptions) error
}
...
func newABCIWrapper(app types.Application) abci.Application {
	return mitosisapp.NewABCIWrapper(
		server.NewCometABCIWrapper(app),
		app.(*mitosisapp.MitosisApp).EVMEngKeeper.PostFinalize,
...
func startStandAlone(svrCtx *server.Context, svrCfg serverconfig.Config, clientCtx client.Context, app types.Application, metrics *telemetry.Metrics, opts StartCmdOptions) error {
	...
	g.Go(func() error {
		...
		// so we can gracefully stop the ABCI cmtserver.
		<-ctx.Done()
		svrCtx.Logger.Info("stopping the ABCI cmtserver...")
		return svr.Stop()
	})
...
func testnetify(ctx *server.Context, testnetAppCreator types.AppCreator, db dbm.DB, traceWriter io.WriteCloser) (types.Application, error) {
	...
	cmtApp := newABCIWrapper(testnetApp)
